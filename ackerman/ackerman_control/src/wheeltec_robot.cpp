...
//    setVelocityCommand(); //eric: Test velocity command without toplic.
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  Cmd_Vel_Sub = n.subscribe("/teleop_velocity_smoother/raw_cmd_vel", 100, &turn_on_robot::Cmd_Vel_Callback, this);//因为官方的平滑包只支持X和W，没有Y，所以这里不使用平滑包
//  Cmd_Vel_Sub = n.subscribe("/ackermann_cmd", 100, &turn_on_robot::Cmd_Vel_Callback, this);//接收阿克曼类型的数据
//Cmd_Vel_Sub = n.subscribe(smoother_cmd_vel, 100, &turn_on_robot::Cmd_Vel_Callback, this);//订阅smoother_cmd_vel话题并控制机器人//差速 
  ...
  //初始化串口
  try{
         Stm32_Serial.setPort("/dev/ttyUSB1");//选择哪个口，如果选择的口没有接串口外设初始化会失败
         Stm32_Serial.setBaudrate(115200);//设置波特率
...
void turn_on_robot::setVelocityCommand()
{
    geometry_msgs::Twist twist_aux;
    twist_aux.linear.x = 5.0;
    twist_aux.linear.y = 0.0;
